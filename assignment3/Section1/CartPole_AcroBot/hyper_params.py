...
class HyperParams:
    ...
    def log_dir(self):
        log_dir = f"hst{self.hidden_sizes_str(self.hidden_sizes_theta)}_hsw{self.hidden_sizes_str(self.hidden_sizes_w)}_at{self.alpha_theta:.4f}_aw{self.alpha_w:.4f}_g{self.gamma:.4f}"
        if self.epsilon is not None:
...
class HyperParamsRanges:
    ...
    def extract_best_hyper_params(best_params, fixed_hidden_theta=None, fixed_hidden_w=None):
        ...
        return HyperParams(hidden_sizes_theta, hidden_sizes_w, alpha_theta_opt, alpha_w_opt, gamma_opt)
